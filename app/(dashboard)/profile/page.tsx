...
//         const recentOnes = (userInspections as any[])
//           .slice(0, 3)
//           .map((inspection) => ({
//             ...inspection,
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setUser(typedUser);
      setAvatarUrl(typedUser.user_metadata?.avatar_url || null);
      ...
      setStats({
        completedInspections: inspections.filter((i) => i.status === 'passed')
          .length,
        pendingInspections: inspections.filter((i) => i.status === 'failed')
          .length,
        totalVehicles: vehicleIds.length,
      });
      ...
      setRecentInspections(
        inspections.slice(0, 3).map((i) => ({
          ...i,
          vehicle: vehicleMap.get(i.vehicle_id),
        })),
      );
    };
  ...
  return (
    <div className="min-h-screen bg-[#f8faff] dark:bg-blue-950">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 grid lg:grid-cols-3 gap-8">
        {/* LEFT */}
        <div className="bg-white dark:bg-card p-6 rounded-xl border">
          <div className="flex flex-col items-center">
            ...
            <h2 className="mt-4 font-bold text-xl">{user.email}</h2>
          ...
          <div className="mt-6 grid grid-cols-3 text-center">
            <div>
              <p>{stats.completedInspections}</p>
              <span>Passed</span>
            </div>
            <div>
              <p>{stats.pendingInspections}</p>
              <span>Failed</span>
            </div>
            <div>
          ...
          <button
            onClick={handleSignOut}
            className="mt-6 w-full flex justify-between p-3 text-red-500"
          >
            Sign out
            <ArrowRightOnRectangleIcon className="w-5 h-5" />
          </button>
        ...
        <div className="lg:col-span-2">
          <h3 className="text-xl font-bold mb-4">Recent inspections</h3>

          <div className="space-y-3">
            {recentInspections.map((i) => (
              <div key={i.id} className="p-4 border rounded-lg">
                <p className="font-bold">{i.vehicle?.regnumber || 'Unknown'}</p>
